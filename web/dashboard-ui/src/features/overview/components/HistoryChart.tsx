...
export default function HistoryChart({ logs }: { logs: SecurityLog[] }) {
  const chartData = useMemo(() => {
    const grouped: Record<string, ChartDataPoint> = {}; // 👈 FIX: No more 'any'

    logs.forEach((log) => {
      const timeKey = new Date(log.timestamp).toLocaleTimeString("en-GB", {
        hour: "2-digit",
        minute: "2-digit",
      });
      ...
      // FIX: Pakai if-else supaya linter senang (no-unused-expressions)
      if (log.is_blocked) {
